...
function buildSearchIndex() {
  ...
  MANUAL_DATA.phases.forEach(phase => {
    idx.push({
      id: `p${phase.id}`, phaseId: phase.id, sectionId: null,
      title: phase.title, subtitle: phase.subtitle, icon: phase.icon,
      color: phase.color, text: `${phase.title} ${phase.subtitle} ${phase.objective} ${phase.weeks}`.toLowerCase(),
      snippet: phase.objective, type: 'phase'
    });
    phase.sections.forEach(sec => {
      let texts = [sec.title];
      if (sec.type === 'table') { sec.rows.forEach(r => texts.push(...r)); if (sec.headers) texts.push(...sec.headers); }
      else if (sec.type === 'cards') { sec.items.forEach(it => texts.push(it.label, it.text)); }
      else if (sec.type === 'list') { sec.items.forEach(it => texts.push(it.title, it.text)); }
      else if (sec.type === 'steps') { sec.steps.forEach(s => { texts.push(s.title); if (s.items) texts.push(...s.items); }); }
      else if (sec.type === 'comparison') { texts.push(sec.sifaTitle, sec.sapTitle, ...(sec.sifa||[]), ...(sec.sap||[]), sec.note||''); }
      else if (sec.type === 'checklist') { texts.push(...sec.items); }
      else if (sec.type === 'zones') { sec.zones.forEach(z => texts.push(z.name, z.desc)); }
      else if (sec.type === 'hierarchy') { sec.levels.forEach(l => texts.push(l.level, ...l.items)); }
      else if (sec.type === 'sap-movements') { sec.movements.forEach(m => texts.push(m.code, m.name, m.desc)); }
      else if (sec.type === 'sap-table') { sec.rows.forEach(r => texts.push(...r)); }
      const combined = texts.filter(Boolean).join(' ');
      idx.push({
        id: `p${phase.id}-s${sec.id}`, phaseId: phase.id, sectionId: sec.id,
        title: sec.title, icon: phase.icon, color: phase.color,
        text: combined.toLowerCase(), snippet: texts.slice(1, 4).join(' › '), type: 'section'
      });
    });
  });

  // Index official guides
  if (typeof GUIAS_OFICIALES !== 'undefined') {
    GUIAS_OFICIALES.categories.forEach(cat => {
      idx.push({
        id: `g-${cat.id}`, guideId: cat.id, sectionId: null,
        title: cat.title, subtitle: cat.subtitle, icon: cat.icon,
        color: cat.color, code: cat.code,
        text: `${cat.title} ${cat.subtitle} ${cat.objective} ${cat.code}`.toLowerCase(),
        snippet: cat.objective, type: 'guide'
      });
      cat.sections.forEach(sec => {
        ...
        const combined = texts.filter(Boolean).join(' ');
        idx.push({
          id: `g-${cat.id}-${sec.id}`, guideId: cat.id, sectionId: sec.id,
          title: sec.title, icon: cat.icon, color: cat.color, code: cat.code,
          text: combined.toLowerCase(), snippet: texts.slice(1, 4).join(' › '), type: 'guide-section'
        });
      });
    });
  }
...
function routeFromHash() {
  ...
  } else if (parts[0] === 'guia' && parts[1]) {
    navigateGuide(parts[1], parts[2] || null);
  } else {
    navigate('home');
  }
}
...
function renderHome() {
  ...
  home.insertAdjacentHTML('beforeend', `
    ...
    ">
      ...
      <p style="margin-top:10px">Elaborado con el <strong style="color:var(--text-primary)">Apoyo de Nelson Fallas Rodríguez</strong>
      Jefatura ALM Salud Goicoechea 1 .</p>
  ...
  // Official CCSS Guides section
  if (typeof GUIAS_OFICIALES !== 'undefined' && GUIAS_OFICIALES.categories.length > 0) {
    home.insertAdjacentHTML('beforeend', `<div class="section-head fade-up-3" style="margin-top:36px"><h2>📋 Guías Operativas Oficiales — CCSS SAP</h2></div>`);
    const ogrid = document.createElement('div');
    ogrid.className = 'phases-grid fade-up-3';
    GUIAS_OFICIALES.categories.forEach(cat => {
      const card = document.createElement('div');
      card.className = 'phase-card';
      card.style.cssText = `border-top:3px solid ${cat.color}`;
      card.innerHTML = `
        <div class="pc-header">
          <span class="pc-icon">${safeText(cat.icon)}</span>
          <span class="pc-num" style="background:${cat.color}22;color:${cat.color};font-size:0.62rem">${safeText(cat.code)}</span>
          <span class="pc-weeks">${safeText(cat.version.split('·')[0])}</span>
        </div>
        <div class="pc-title" style="font-size:0.9rem">${safeText(cat.title)}</div>
        <div class="pc-subtitle">${safeText(cat.subtitle)}</div>
        <div style="margin-top:8px">
          <span class="pc-level" style="font-family:var(--font-body);font-size:0.7rem">${cat.sections.length} secciones</span>
        </div>
      `;
      card.onclick = () => navigateGuide(cat.id);
      ogrid.appendChild(card);
    });
    home.appendChild(ogrid);
  }
...
/* ── GUIDE NAVIGATION ───────────────────────────────────────── */
function navigateGuide(guideId, sectionId = null) {
  state.view = 'guide';
  state.phaseId = guideId;
  state.sectionId = sectionId;
  renderGuide(guideId);
  history.replaceState(null, '', `#guia/${guideId}${sectionId ? '/' + sectionId : ''}`);
}

function renderGuide(guideId) {
  if (typeof GUIAS_OFICIALES === 'undefined') return;
  const cat = GUIAS_OFICIALES.categories.find(c => c.id === guideId);
  ...
  view.innerHTML = `
    <div class="phase-header fade-up" style="position:relative">
      <div style="position:absolute;top:0;left:0;right:0;height:3px;background:${cat.color}"></div>
      <div class="ph-row">
        <span class="ph-icon">${safeText(cat.icon)}</span>
        <div class="ph-meta">
          <div class="ph-label">${safeText(cat.code)} · ${safeText(cat.version)}</div>
          <div class="ph-title">${safeText(cat.title)}</div>
          <div class="ph-subtitle">${safeText(cat.subtitle)}</div>
        </div>
      </div>
      <div class="ph-chips">
        <span class="ph-chip" style="color:${cat.color};border-color:${cat.color}44">📋 Guía Oficial CCSS</span>
        <span class="ph-chip">${cat.sections.length} secciones</span>
      </div>
      <div class="ph-objective" style="border-left-color:${cat.color}">
        <strong>Objetivo:</strong> ${safeText(cat.objective)}
      </div>
    </div>
  `;
  ...
  cat.sections.forEach((sec, i) => {
    ...
    card.innerHTML = `
      <div class="sec-card-header" onclick="toggleSection('${sec.id}')">
        <span class="sec-card-id" style="background:${cat.color}22;color:${cat.color}">${safeText(sec.id)}</span>
        <span class="sec-card-title">${safeText(sec.title)}</span>
        <svg class="sec-card-toggle" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg>
      </div>
      <div class="sec-card-body" id="sec-body-${sec.id}">
        ${renderSection(sec, cat)}
      </div>
    `;
    view.appendChild(card);
  });
  ...
  backBtn.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 18px;background:var(--bg-card);border:1px solid var(--border-md);border-radius:var(--radius);font-size:0.83rem;color:var(--text-secondary);cursor:pointer;margin:24px 0 40px;transition:all 0.2s';
  ...
  if (state.sectionId) requestAnimationFrame(() => scrollToSection(state.sectionId));
...
function initSearch() {
  ...
  function renderSearchResults(hits, q) {
    ...
    const grouped = { phase: [], section: [], guide: [], 'guide-section': [] };
    hits.forEach(h => { if (grouped[h.type]) grouped[h.type].push(h); else grouped.section.push(h); });

    let html = '';
    if (grouped.phase.length) {
      html += `<div class="sr-header">Fases del Manual</div>`;
      grouped.phase.forEach(h => {
        html += `
          <div class="sr-item" data-phase="${h.phaseId}" data-section="" data-type="phase">
    ...
    if (grouped['guide']) {
      html += `<div class="sr-header">📋 Guías Oficiales CCSS</div>`;
      grouped['guide'].forEach(h => {
        html += `
          <div class="sr-item" data-guide="${h.guideId}" data-section="" data-type="guide">
            <div class="sr-badge" style="background:${h.color}22;font-size:14px">${h.icon}</div>
            <div class="sr-info">
              <div class="sr-title">${markText(h.title, q)}</div>
              <div class="sr-meta">${safeText(h.code || '')} · Guía Oficial</div>
    ...
    const sectionTypes = ['section', 'guide-section'];
    const sectionHits = hits.filter(h => sectionTypes.includes(h.type));
    if (sectionHits.length) {
      html += `<div class="sr-header">Secciones (${sectionHits.length})</div>`;
      sectionHits.slice(0, 10).forEach(h => {
        const isGuide = h.type === 'guide-section';
        html += `
          <div class="sr-item" data-phase="${h.phaseId||''}" data-guide="${h.guideId||''}" data-section="${h.sectionId}" data-type="${h.type}">
            <div class="sr-badge" style="background:${h.color}22;font-size:14px">${h.icon}</div>
            <div class="sr-info">
              <div class="sr-title">${markText(h.title, q)}</div>
              <div class="sr-meta">${isGuide ? `📋 ${safeText(h.code||'')}` : `Fase ${h.phaseId}`} · ${safeText(h.sectionId)}</div>
              <div class="sr-snippet">${safeText(h.snippet.substring(0,90))}</div>
            </div>
          </div>
        `;
    ...
    results.querySelectorAll('.sr-item').forEach(el => {
      el.addEventListener('click', () => {
        ...
        const guideId   = el.dataset.guide;
        const phaseId   = parseInt(el.dataset.phase);
        const sectionId = el.dataset.section || null;
        closeSearch();
        input.value = '';
        clearBtn.classList.remove('visible');
        if (type === 'guide' || type === 'guide-section') {
          navigateGuide(guideId, sectionId);
        } else {
